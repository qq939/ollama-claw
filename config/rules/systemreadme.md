...
模型管理采用容器卡片级部署，不再由全局模型管理批量部署到所有容器。

控制面板的每个容器卡片可以执行“拉取并应用模型”：

  1) 触发 Ollama 拉取指定模型
  2) 写入当前容器自己的配置文件
  3) 重启当前容器

Claude/Hermes 容器的 base url 应指向 Docker 网络中的 Ollama：

  ANTHROPIC_BASE_URL=http://ollama:11434
  ANTHROPIC_AUTH_TOKEN=ollama
  ANTHROPIC_API_KEY=ollama

OpenClaw 容器使用 `~/.openclaw/openclaw.json` 中的
`agents.defaults.model.primary`。Claude/Hermes 容器使用各自 home 目录下的
`settings.json` 与 `config.json`。
...
Claude/Hermes 类型默认预设：
  ANTHROPIC_BASE_URL=http://ollama:11434
  ANTHROPIC_AUTH_TOKEN=ollama
  ANTHROPIC_API_KEY=ollama
  OLLAMA_MODEL=qwen2.5:0.5b

claude/hermes 类型可在 /agent-config/settings.json 和 config.json 中配置：
